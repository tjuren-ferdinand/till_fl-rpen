...
import { actions, DEFAULT_GRADING_PARAMS, type Klass, type Kurs } from "@/lib/store";
...
export default function GradingWizard({
  ...
  klass: Klass;
  /** Sätts för kursprov — provet kopplas till kursen istället för klassen. */
  kurs?: Kurs;
  open: boolean;
  onClose: () => void;
  onStarted: (provId: string) => void;
}) {
  // Ägare: klass som standard, kurs i kursprovsläge. Bestämmer lagring,
  // parametrar och var granskningsvyn öppnas.
  const owner = kurs ?? klass;
  const gradingParams = kurs ? DEFAULT_GRADING_PARAMS : klass.gradingParams;
  const gradeBasePath = kurs ? `/courses/${kurs.id}` : `/classes/${klass.id}`;
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      // Skapa provet först — mot kursen i kursprovsläge, annars klassen.
      const prov = await actions.startProv({
        ...(kurs ? { kursId: kurs.id } : { klassId: klass.id }),
    ...
    return (
      <BatchGradingPipeline
        ...
        onOpenResult={(resultId) =>
          router.push(`${gradeBasePath}/grade/${createdProvId}?student=${resultId}`)
        }
      />
    );
  }
        ...
        <header className="border-b border-ink-hairline px-5 pb-3 pt-[max(1.5rem,env(safe-area-inset-top))] sm:px-8 sm:pt-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="mt-1 text-xl font-semibold tracking-tight text-ink">{owner.name}</div>
              {((owner as any).code && (owner as any).code !== owner.name) || (owner.students?.length ?? 0) > 0 ? (
                <div className="mt-1 text-xs font-medium text-ink-muted tracking-wide uppercase">
                  {(owner as any).code && (owner as any).code !== owner.name
                    ? (owner as any).code
                    : `${owner.students.length} elever`}
                </div>
              ) : null}
            </div>
