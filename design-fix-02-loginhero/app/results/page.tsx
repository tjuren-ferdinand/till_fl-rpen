      ...
      {publishedResults.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {publishedResults.map((r) => {
            ...
            // Ägare: klass för klassprov, kurs för kursprov.
            const klass = klasser.find((k) => k.id === provData?.klassId);
            const kurs = !klass ? kurser.find((k) => k.id === provData?.kursId) : undefined;
            const ownerName = klass?.name ?? kurs?.name;
            ...
            return (
              <Surface key={r.id} padding="p-5">
                <div className="text-[11px] uppercase tracking-[0.1em] font-medium text-ink-muted truncate">
                  {ownerName || "Okänd klass"} · {provData?.title || "Okänt prov"}
                </div>
