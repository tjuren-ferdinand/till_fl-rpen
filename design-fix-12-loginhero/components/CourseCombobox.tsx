...
import { actions, deriveSubject, resolveKurs, useStore, KURSKATALOG, type Kurs } from "@/lib/store";
...
export default function CourseCombobox({ value, onSelect, placeholder = "Sök eller skriv kursnamn..." }: CourseComboboxProps) {
  ...
  // Egna kurser först, därefter katalogförslag (GY11) — deduplicerat på id.
  const allOptions = useMemo(
    () => [
      ...kurser,
      ...KURSKATALOG.filter((c) => !kurser.some((k) => k.id === c.id)),
    ],
    [kurser],
  );
  const selected = resolveKurs(kurser, value);

  const [inputValue, setInputValue] = useState(selected?.name || "");
  const [isOpen, setIsOpen] = useState(false);
  const [filtered, setFiltered] = useState<Kurs[]>(allOptions);
