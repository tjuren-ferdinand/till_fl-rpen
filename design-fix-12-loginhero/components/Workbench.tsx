...
export default function Workbench({ result, prov, klass, onBack, onPrint, allResults, onNavigate }: Props) {
  ...
  // Basväg beror på provets ägare — klassprov under /classes, kursprov
  // under /courses (samma vy, delad komponent).
  const basePath = prov.kursId ? `/courses/${prov.kursId}` : `/classes/${klass.id}`;

  const handlePrint = () => {
    router.push(`${basePath}/grade/${prov.id}/print?student=${result.id}`);
  };
  ...
  return (
    <>
      <div className="space-y-6 print:hidden">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            >
              {prov.kursId ? "Till kursmapp" : "Till klassmapp"}
            </button>
          </div>
          <div className="flex items-center gap-3">
            <button
      ...
      <Surface className="space-y-4 !shadow-card">
        <Breadcrumb items={[
          { label: klass.name, href: basePath },
          { label: prov.title, href: `${basePath}/grade/${prov.id}` },
