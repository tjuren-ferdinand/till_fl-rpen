...
import { useStore, deriveStep, actions, DEFAULT_GRADING_PARAMS, type Klass, type StudentResult } from "@/lib/store";
...
/**
 * Delad granskningsvy för prov — oavsett om provet hör till en klass
 * (`/classes/[id]/grade/[provId]`) eller direkt till en kurs
 * (`/courses/[id]/grade/[provId]`). Ägaren mappas till ett Klass-liknande
 * objekt så att Workbench, deriveStep och ProcessingScene fungerar identiskt.
 */
export default function ProvGradeScreen({ ownerKind }: { ownerKind: "klass" | "kurs" }) {
  ...
  // Ägaren: en riktig klass, eller en kurs mappad till samma form.
  const owner = useMemo(() => {
    if (ownerKind === "klass") return klasser.find((k) => k.id === params.id);
    const kurs = kurser.find((k) => k.id === params.id);
    if (!kurs) return undefined;
    const klassLike: Klass = {
      id: kurs.id,
      name: kurs.name,
      kursId: kurs.id,
      students: kurs.students,
      gradingParams: { ...DEFAULT_GRADING_PARAMS },
      gradeThresholds: kurs.gradeThresholds,
    };
    return klassLike;
  }, [ownerKind, klasser, kurser, params.id]);
  ...
  const basePath = ownerKind === "kurs" ? `/courses/${params.id}` : `/classes/${params.id}`;
  const ownerLabel = ownerKind === "kurs" ? "Kursmapp" : "Klassmapp";
  ...
  if (!owner || !prov) {
    return (
      <EmptyState
        ...
        action={<Link href={ownerKind === "kurs" ? "/courses" : "/classes"} className="btn-secondary">{ownerKind === "kurs" ? "Till kurslistan" : "Till klasslistan"}</Link>}
      />
    );
  }
  ...
  if (studentId) {
    ...
    return (
      <Workbench
        ...
        onNavigate={(id) => router.push(`${basePath}/grade/${prov.id}?student=${id}`)}
        onBack={() => router.push(`${basePath}/grade/${prov.id}`)}
    ...
    return (
      <div className="-mx-5 -mt-6 flex min-h-[calc(100vh-8rem)] items-center justify-center px-5">
        <div className="w-full max-w-2xl">
          <ProcessingScene prov={prov} klass={owner} onBack={() => router.push(basePath)} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 print:hidden">
      <div className="space-y-4">
        <Breadcrumb items={[
          { label: owner.name, href: basePath },
          { label: prov.title },
        ]} />
        <PageHeader
          title={prov.title}
          subtitle={`${ownerLabel} · ${allResults.filter((r) => r.document?.documentType !== "not_student_submission").length} elever rättade`}
        />
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {allResults.map((r) => {
            const total = r.steps.reduce(
              (s, st) => s + deriveStep(st, owner.gradingParams).earnedPoints,
              0,
            );
            ...
            return (
              ...
              >
                <Surface
                  href={`${basePath}/grade/${prov.id}?student=${r.id}`}
                  padding="p-5"
        ...
        return (
          <>
            ...
            >
              ...
              <button
                onClick={() => { setMenuFor(null); router.push(`${basePath}/grade/${prov.id}?student=${target.id}`); }}
                className="w-full rounded-lg px-3 py-2 text-left text-sm text-ink hover:bg-ink/5"
              >
                Öppna granskning
              </button>
