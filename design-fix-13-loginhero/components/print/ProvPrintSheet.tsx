...
/**
 * Delad utskriftsvy för ett elevresultat — fungerar för både klassprov
 * (`/classes/[id]/grade/[provId]/print`) och kursprov
 * (`/courses/[id]/grade/[provId]/print`). Ägaren (klassens eller kursens
 * namn) slås upp via provets egen koppling.
 */
export default function ProvPrintSheet() {
  ...
  const kurser = useStore((st) => st.kurser);
  ...
  // Ägaren slås upp via provets koppling — klassprov via klassId,
  // kursprov via kursId. params.id används som fallback.
  const owner = useMemo(() => {
    const klass = klasser.find((k) => k.id === (prov?.klassId ?? params.id));
    if (klass) return { name: klass.name, href: `/classes/${klass.id}` };
    const kurs = kurser.find((k) => k.id === (prov?.kursId ?? params.id));
    if (kurs) return { name: kurs.name, href: `/courses/${kurs.id}` };
    return undefined;
  }, [klasser, kurser, prov, params.id]);
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.meta}>
        {owner.name} · {prov.title} · Provdatum {provDate}
      </div>
