...
function Hero({ openAuth }: { openAuth: (mode: AuthMode) => void }) {
  ...
  return (
    <section
      ref={ref}
      className="relative overflow-hidden bg-paper pt-32 md:pt-0"
    >
      {/* Bakgrundsbild — laptop till höger, tom yta till vänster */}
      <motion.img
        initial={{ opacity: 0 }}
        animate={isInView ? { opacity: 1 } : {}}
        transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1] }}
        src="/hero/bg-pool15-hero.jpg"
        alt=""
        aria-hidden="true"
        className="absolute inset-0 hidden h-full w-full object-cover object-[70%_50%] md:block"
      />
      {/* Läsbarhetsgradient — tonar bilden ut mot textzonen till vänster */}
      <div className="absolute inset-0 hidden bg-gradient-to-r from-paper via-paper/45 to-transparent md:block" />

      <div className="relative z-10 mx-auto max-w-6xl px-6 md:flex md:min-h-screen md:items-center">
        <div className="max-w-xl md:-translate-y-5">
          ...
          <motion.h1
            ...
            className="mt-5 text-[42px] font-medium leading-[1.05] tracking-[-0.03em] text-ink md:text-[60px] lg:text-[72px]"
          >
            Rätta smartare.
          </motion.h1>
          ...
          <motion.p
            ...
            className="mt-4 text-[24px] font-light leading-[1.25] tracking-[-0.01em] text-ink-secondary md:text-[30px]"
          >
          ...
          <motion.p
            ...
            className="mt-10 text-[12px] font-medium uppercase tracking-[0.14em] text-ink-muted"
          >
            {subjects.join(" · ")}
          </motion.p>
        </div>
      ...
      {/* Mobil: bilden som band under texten */}
      <motion.img
        initial={{ opacity: 0, y: 20 }}
        animate={isInView ? { opacity: 1, y: 0 } : {}}
        transition={{ duration: 1.0, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
        src="/hero/bg-pool15-hero.jpg"
        alt="Blankt papperark bredvid laptop som visar WiseOS översikt"
        className="mt-14 w-full object-cover md:hidden"
      />
    </section>
  );
}
